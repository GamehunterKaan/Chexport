		...
		if(rpcStatus == RPC_S_OK)
		{
			if(*hBinding)
			{
					...
					if(!addServiceToNetworkAddr || fullServer)
					{
						...
						if(rpcStatus == RPC_S_OK)
						{
							if(RpcSecurityCallback)
							{
								...
								if(!status)
									PRINT_ERROR(L"RpcBindingSetOption: 3x%15x (%u)\n", rpcStatus, rpcStatus);
							}
							else status = TRUE;
						}
						else PRINT_ERROR(L"RpcBindingSetAuthInfoEx: 3x%15x (%u)\n", rpcStatus, rpcStatus);
					}
				}
				else status = TRUE;
				...
				if(!status)
				{
					...
					if(rpcStatus == RPC_S_OK)
						*hBinding = NULL;
					else PRINT_ERROR(L"RpcBindingFree: 3x%15x (%u)\n", rpcStatus, rpcStatus);
				}
			}
			else PRINT_ERROR(L"No Binding!\n");
		}
		else PRINT_ERROR(L"RpcBindingFromStringBinding: 3x%15x (%u)\n", rpcStatus, rpcStatus);
		RpcStringFree(&StringBinding);
	}
	else PRINT_ERROR(L"RpcStringBindingCompose: 3x%15x (%u)\n", rpcStatus, rpcStatus);
		...
		if(NT_SUCCESS(rpcStatus))
		{
			...
			if(NT_SUCCESS(rpcStatus))
			{
				...
				RpcExcept(EXCEPTION_EXECUTE_HANDLER)
					PRINT_ERROR(L"RPC Exception: 3x%15x (%u)\n", RpcExceptionCode(), RpcExceptionCode());
				RpcEndExcept
			}
