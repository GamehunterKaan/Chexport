...
BOOL kull_m_crypto_NCryptGetProperty(NCRYPT_HANDLE monProv, LPCWSTR pszProperty, BOOL withError, PBYTE *data, OPTIONAL DWORD *cbData, OPTIONAL DWORD *simpleDWORD, OPTIONAL NCRYPT_HANDLE *simpleHandle)
{
	...
	if(simpleDWORD)
	{
		...
		if(NT_SUCCESS(ntStatus))
			status = TRUE;
		else if(withError) PRINT_ERROR(L"NCryptGetProperty(%s) - simple DWORD: 3x%15x\n", pszProperty, ntStatus);
	}
	else if(simpleHandle)
	{
		...
		if(NT_SUCCESS(ntStatus))
			status = TRUE;
		else if(withError) PRINT_ERROR(L"NCryptGetProperty(%s) - simple NCRYPT_HANDLE: 3x%15x\n", pszProperty, ntStatus);
	...
	else
	{
		...
		if(NT_SUCCESS(ntStatus))
		{
			if(*data = (PBYTE) LocalAlloc(LPTR, dwSizeNeeded))
			{
				...
				{
					if(withError)
						PRINT_ERROR(L"NCryptGetProperty(%s) - data: 3x%15x\n", pszProperty, ntStatus);
					*data = (PBYTE) LocalFree(*data);
				}
			}
		}
		else if(withError) PRINT_ERROR(L"NCryptGetProperty(%s) - init: 3x%15x\n", pszProperty, ntStatus);
	}
	return status;
}
...
BOOL kull_m_crypto_NCryptFreeHandle(NCRYPT_PROV_HANDLE *hProv, NCRYPT_KEY_HANDLE *hKey)
{
	...
	if(*hKey)
	{
		__try
		{
			...
			if(NT_SUCCESS(ntStatus))
				*hKey = 0;
			else PRINT_ERROR(L"NCryptFreeObject(key): 3x%15x\n", ntStatus);
		}
	...
	if(*hProv)
	{
		__try
		{
			...
			if(NT_SUCCESS(ntStatus))
				*hProv = 0;
			else PRINT_ERROR(L"NCryptFreeObject(prov): 3x%15x\n", ntStatus);
		}
		__except(GetExceptionCode() == ERROR_DLL_NOT_FOUND)
		{
			PRINT_ERROR(L"No CNG to support this function\n");
		...
		if(NT_SUCCESS(ntStatus))
		{
			...
			if(NT_SUCCESS(ntStatus))
			{
				...
				if(NT_SUCCESS(ntStatus))
				{
					...
					if(NT_SUCCESS(ntStatus))
						status = TRUE;
					else PRINT_ERROR(L"NCryptFinalizeKey: 3x%15x\n", ntStatus);
				}
				else PRINT_ERROR(L"NCryptSetProperty(export_policy): 3x%15x\n", ntStatus);
			}
			else PRINT_ERROR(L"NCryptImportKey: 3x%15x\n", ntStatus);
		}
		else PRINT_ERROR(L"NCryptOpenStorageProvider: 3x%15x\n", ntStatus);
		if(!status)
